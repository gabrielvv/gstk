...
var forEach = function(arr, fn){ return Array.prototype.forEach.call(arr, fn); };
...
var Graph = (function(global){
  ...
  Graph.prototype.print = function() {
    (console.log || Logger.log)(this.vertices.map(function(vertex) {
...
*
*/
var modularize = (function(global, Graph, annotate){
  "use strict";
  ...
  */
  ...
  Module.prototype.factory = function(serviceName, factory){
    var self = this;
  ...
  *
  */
  modularize.module = function(modName, requires){

     Logger.log("modularize.module %s %s", modName, requires);
    ...
    // module retrieval
    if(!this._modules[modName]) throw new Error("cannot find module \""+modName+"\"");
  ...
  */
  var instantiateService = function(mod, service){
    ...
    var args = service.requires
      .map(function(sName){
        Logger.log("sname: "+sName);
        if(modularize._scache[sName]){
          Logger.log("service "+sName+" retrieved from cache");
        }
        return modularize._scache[sName] || instantiateService(mod, mod.factories[sName]);
      })

    var instance = service.fn.apply(service.fn, args);
    modularize._scache[service.name] = instance;
    return instance;
  }
  ...
  */
  var instantiateMod = function(mod){
    ...
    mod.requires.map(function(modName){
      if(modularize._mcache[modName]){
        Logger.log("mod "+modName+" retrieved from cache");
      }
      return modularize._mcache[modName] || instantiateMod(modularize._modules[modName]);
    })
    ...
    return modularize._mcache[mod.name] = 1;
  }

  modularize._scache = {};
  modularize._mcache = {};

  modularize.bootstrap = function(){
    for(var key in modularize._modules){
      instantiateMod(modularize._modules[key]);
    }
  }

  modularize.id = Utilities.getUuid();
  return modularize;
