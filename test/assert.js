  ...
  TestRunner(function(){
    describe("assert", function(){
      ...
      it("assert.isArray", function(){
        ...
        ["lol", null, undefined, function(){}, 123, false].forEach(function(value){
          assert.throw(assert.isArray.bind(this, value), "AssertError");
        });
      });

      it("assert.isFunction", function(){
        assert.not.throw(assert.isFunction.bind(this, function(){}), "AssertError");
      ...
      it("assert.equal", function(){
       assert.throw(assert.equal.bind(this, {a: "a"}, {a: "b"}), "AssertError");
       assert.not.throw(assert.equal.bind(this, {a: "a"}, {a: "a"}), "AssertError");
      });

      it("assert.throw", function(){
        assert.throw(assert.throw.bind(this, function(){}), "AssertError");
        assert.not.throw(assert.throw.bind(this, function(){ throw "UTestError"; }), "AssertError");
      });
    });
  });
}//test_assert
