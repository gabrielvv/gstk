...
(function(modularize){
  ...
  'use strict';
  modularize.module("gstk", ["gs"]).factory("assert", ["Logger", function assertFactory(Logger){

  var assert = function(condition, message){
    if(!condition) throw myNewError("AssertError", "Assertion failed :" + message);
    return condition;
  };
  ...
  */
  assert.equal = function(actual, expected, message, log){
    ...
    if(('object' === typeof actual && actual !== null) && ('object' === typeof expected && actual !== null)){
      if(log){
        Logger.log("\t------------------------------------------------------");
        Logger.log("\t"+actual);
        Logger.log("\t"+expected);
      }
      if(actual.length == expected.length){ /** ATTENTION: typeof null == object */
        var keys =  Object.keys(actual);
        var l = keys.length;

        for(var i = 0; i < l ; i++){
          var key = keys[i];
          if(!assert.equal(actual[key], expected[key])){ i--; break; } /** foireux */
        }
        b = (i === l) ? true : false;
      }else{
        b = false;
    ...
    if(!b) throw myNewError( "AssertError", "\tAssertion failed :" + (message || (actual+"") + " === " + (expected+"")) );
    return b;
  }

  assert.isDefined = function(actual, message){
    var b = (actual !== undefined);
    if(!b) throw myNewError("AssertError", "\tAssertion failed :" + (message || (actual+" undefined")) );
  }

  assert.isFunction = function(actual, message){
    var b = ('function' === typeof actual);
    if(!b) throw myNewError("AssertError", message || ("\tAssertion failed :" + actual + " is a function"));
  }

  assert.isObject = function(actual, message){
    var b = "object" === typeof actual && !!actual;
    if(!b) throw myNewError("AssertError", message || ("\tAssertion failed :" + actual + " is an object"));
  }

  assert.isArray = function(actual, message){
    var b = Array.isArray(actual);
    if(!b) throw myNewError("AssertError", message || ("\tAssertion failed :" + actual + " is an array"));
  }
  ...
  assert.throw = function(fn, errorName, args){
    ...
    if(!b || !err) throw myNewError("AssertError", "\tAssertion failed :" + fn + " does not throw "+errorName, {
...
//      Logger.log("InternalError");
  ...
  assert.hasBeenCalledTimes = function(fn, times){
    ...
    assert(spy.count == times, spy.name + " HasBeenCalledTimes("+times+") +("+spy.count+" counted)");
  }
  ...
  */
  assert.not = function(condition, message){
    ...
    if(err.message.indexOf("InternalError") >= 0) throw myNewError("AssertError", "Assertion failed :" + err.message);
  ...
  */
  function notFactory(fn, name){
    return function(){
      ...
      try{
        var data = fn.apply(null/* dans ce cas, revient au même avec 'global' */, arguments);
      }catch(e){
        err = e;
        if(err.message.indexOf("InternalError") >= 0) throw myNewError("AssertError", "Assertion failed :" + err.message);
        return err.data;
      ...
      if(!err) throw myNewError("AssertError", "Assertion failed : not." + name + (name == 'throw' ? " - " + data.message : ""));
      return data;
    }
  }//notFactory
